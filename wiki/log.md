## 2026-06-06 — Wire minimal 5-min Decision Report generator (additive spawn in main + JournalWriter extension for 'decision_report' reuse events; real FusionEngine::fuse_net + DecisionReport net_edge primary per strategy/goals skeleton; hermes now COUNTs real replacing 0 stub + updates cadence/attribution/summary/recs; makes self-imp data (DR cadence + approvals) actionable + advances wiki-tracked 5-min DR "Trader" cadence (heavily in goals "Extend do_reflection...", plan post-stub, log "Ready for next / backtest", strategy "PRIMARY signal for 5-min tier"); wiki-first to existing files only; no UI/SSR/deploy change; 100% prior surfaces (incl all polish/DR-stub markers + paper/fail-closed/gated/L2/SSR subpath+<base>+every old) preserved; local cargo green; smallest per AGENTS)
