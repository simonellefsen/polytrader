...
- `clob_order_human_approval`: short-lived, journaled human approval workflow event keyed to a deterministic order-intent subject hash. Enriched (2026-06-03 approval UX) with risk_snapshot_at_approval (projected notional/limits for the intent) + collateral_snapshot_at_approval (from builders) + operator (from AuthUser) + approval_time. Used to satisfy human_approval_event_id gate in submit-facade + GatedRealClobLiveOrderSender reval for real CLOB path (when unlocks+kill+valid non-zero id). Still expires; "approve_facade" decision required by validate; does not itself enable real (all other gates + reval still apply). `request_sent=false` etc always in base.
- `clob_order_submit_facade`: fail-closed real-order submission facade evaluation. It records gate status for human approval, fresh collateral-readiness checkpoint, kill switch, per-order exposure, total exposure, daily loss, paper mode, and explicit config unlocks, but must always record `request_sent=false` while real trading is disabled. (Now surfaces approval-time snapshots from linked events for staleness/reval attribution.)
...
- `clob_live_sender_boundary_status`: read-only status for the code-level live-sender trait boundary. It records that `LiveOrderSender` exists, the only implementation is `FailClosedLiveOrderSender`, `network_sender_present=false`, `accepted_for_network_dispatch=false`, `request_sent=false`, and no order POST calls. (Gated real sender is present behind boundary but not default.)
...
- `clob_final_review_decision`: audit-only operator decision linked to a `clob_final_review_readiness` event. Enriched (2026-06-03) with risk/collateral snapshots at decision time + operator binding. Records `acknowledge_blocked`, `reject_live_trading`, or `needs_rework` decisions. Non-zero id (any valid recorded decision per operator judgment) satisfies final_review_decision_event_id gate for submit-facade + Gated reval (real path still requires unlocks+kill+human+all risk/collateral at facade time; `approved_for_real_orders=false` in base payload, `request_sent=false`). Hermes consumes for safety loop.

Payloads must not contain secrets, raw private keys, or L2 API secrets. Approval/decision events provide human-in-the-loop evidence + at-approve snapshots for the gated real CLOB path but do not auto-approve, open kill switches, or bypass risk/collateral/unlock revals at dispatch time. They remain journaled audit records; real dispatch requires every gate + hard pre-dispatch journal.
...
## 2026-06-03 Approval UX Tranche Notes (no schema migration)
- Enriched payloads only (jsonb in journal.events): `clob_order_human_approval` and `clob_final_review_decision` now embed `risk_snapshot_at_approval` / `collateral_snapshot_at_approval` (and operator/approval_time) captured at POST time via existing builders + intent-derived risk. No new columns, no new tables, no migration.
- Existing queries (validate_*, hermes counts on event_type, latest_journal_event, fetch_*) unaffected (forward compat).
- See wiki/decisions/real-order-approval-flow.md , wiki/log.md (2026-06-03 entry), and updated event descriptions above. Hermes attribution and reval at dispatch use the embedded snapshots for evidence (no code change to queries).
- All per AGENTS (wiki/schema first for event shape evolution).
